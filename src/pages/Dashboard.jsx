...
function currentFY() {
  const now = new Date();
  return now.getMonth() >= FY_START_MONTH
    ? now.getFullYear()
    : now.getFullYear() - 1;
}
function fyRange(fy) {
  return {
    start: new Date(fy, FY_START_MONTH, 1),
    end: new Date(fy + 1, FY_START_MONTH, 1),
  };
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    api
      .adminEnquiries({ page: 0, size: 6 })
      .then((d) => setRecentEnquiries(d.content || []))
      .catch(() => {});
    api
      .adminEnquiries({ status: "NEW", page: 0, size: 1 })
      .then((d) => setNewEnquiryCount(d.totalElements || 0))
      .catch(() => {});
    api
      .quotations({ page: 0, size: 1 })
      .then((d) => setQuotationCount(d.totalElements || 0))
      .catch(() => {});
    api
      .adminEnquiries({ status: "WORK_ORDER", page: 0, size: 1 })
      .then((d) => setWorkOrderCount(d.totalElements || 0))
      .catch(() => {});
  }, []);
  ...
  const kpis = useMemo(() => {
    ...
    const income = active.reduce(
      (s, a) => s + (parseFloat(a.amcAmount) || 0),
      0,
    );
    ...
    return {
      ...
      due30: due30.length,
      svcDue30: svcDue30.length,
      expired,
      total: allAmcs.length,
    };
  ...
  const monthlyData = useMemo(() => {
    ...
    return MONTHS.map((m, i) => {
      ...
      const income = allAmcs
        .filter(
          (a) =>
            a.status === "ACTIVE" &&
            inMonth(parseDate(a.startDate), calYear, calMonth),
        )
    ...
    <section className="page">
      ...
      <div className="page-head">
        <div>
          ...
          <h1>Dashboard</h1>
          <p className="muted">
            Apollo Elevator · service operations at a glance
          </p>
        </div>
        <Link className="primary" to="/app/customers/new">
          + Add customer
        </Link>
      </div>
      ...
      <div className="dash-filter">
        <div className="segmented" style={{ marginBottom: 0, width: "auto" }}>
          <button
            className={filterMode === "fy" ? "selected" : ""}
            onClick={() => setFilterMode("fy")}
          >
            By Financial Year
          </button>
          <button
            className={filterMode === "month" ? "selected" : ""}
        ...
        {filterMode === "fy" ? (
          <div className="segmented" style={{ marginBottom: 0, width: "auto" }}>
            {fyYears.map((y) => (
              <button
                key={y}
                className={selectedFY === y ? "selected" : ""}
      ...
      <div className="kpi-grid">
        <KpiCard
          ...
          sub={filterMode === "fy" ? fyLabel(selectedFY) : "This month"}
          color="blue"
        />
        <KpiCard
          icon={CheckCircle}
      ...
      <div className="charts-row">
        <div className="panel" style={{ flex: 2 }}>
          <div className="panel-head">
            <h2>
              Monthly AMC Income (
              {fyLabel(filterMode === "fy" ? selectedFY : currentFY())})
            </h2>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            ...
            >
              ...
              />
              <Tooltip
                formatter={(v) => [`₹${v.toLocaleString("en-IN")}`, "Income"]}
              />
      ...
      <div className="due-row">
        <div className="panel" style={{ flex: 1 }}>
          <div className="panel-head">
            <h2>💳 AMC Payments due (next 30 days)</h2>
          </div>
          <DueTable
            rows={amcDueList}
            dateKey="nextPaymentDate"
            emptyMsg="No payments due in next 30 days"
          />
        </div>
        <div className="panel" style={{ flex: 1 }}>
          <div className="panel-head">
            <h2>🔧 Services due (next 30 days)</h2>
          </div>
          <DueTable
            rows={svcDueList}
            dateKey="nextServiceDate"
            emptyMsg="No services due in next 30 days"
          />
        </div>
      </div>
        ...
        <div className="panel-head">
          <div>
            ...
            <span className="muted">
              Contact & quote requests from the website
            </span>
          </div>
          <Link
            to="/app/enquiries"
            ...
            <tbody>
              {recentEnquiries.map((i) => (
                <tr key={i.id}>
                  ...
                  <td>
                    <span
                      className={`status ${i.status === "NEW" ? "pending" : i.status === "CLOSED" ? "active" : ""}`}
                    >
                      {i.status?.replace("_", " ")}
                    </span>
                  </td>
                  ...
                  <td>
                    <Link className="table-link" to="/app/enquiries">
                      Respond
                    </Link>
                  </td>
                </tr>
          ...
          <table>
            ...
            <tbody>
              {allCustomers.slice(0, 6).map((c) => {
                const activeAmc = (c.lifts || [])
                  .flatMap((l) => l.amcDetails || [])
                ...
                return (
                  <tr key={c.id}>
                    ...
                    <td>
                      <span
                        className={`status ${activeAmc ? "active" : "pending"}`}
                      >
                        {activeAmc}
                      </span>
                    </td>
...
function KpiCard({ icon: Icon, label, value, sub, color }) {
  ...
  return (
    <div className="kpi-card">
      <div className="kpi-icon" style={{ background: bg, color: fg }}>
        <Icon size={20} />
      </div>
      <div>
            ...
            return (
              <tr key={i}>
                <td>
                  <Link
                    className="table-link"
                    to={`/app/customers/${a.customerId}`}
                  >
                    <b>{a.customerName}</b>
                  </Link>
                </td>
                <td>
                  <small>{a.contractNumber || "—"}</small>
                </td>
                <td>{d ? d.toLocaleDateString("en-IN") : "—"}</td>
                <td>
                  <span
                    className={`status ${days <= 7 ? "expired" : days <= 14 ? "pending" : "active"}`}
                  >
                    {days != null ? `${days}d` : "—"}
                  </span>
                </td>
